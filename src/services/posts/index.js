...
postRouter.get("/", async (req, res, next)=>{
    try {
        const allPosts = await PostModel.find().populate({path:'Comments',populate:{path:'user_id',model:"Users"}}).populate("user_id")
...
postRouter.get("/:id", async (req, res, next)=>{
    try {
        const allPosts = await PostModel.findById(req.params.id).populate({path:'Comments',populate:{path:'user_id',model:"Users"}}).populate("user_id")
        res.send.status(201).send(allPosts)
